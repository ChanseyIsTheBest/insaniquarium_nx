...
# Imported from the upstream Insaniquarium PortMaster port, which derives the
# decompilation into a directory it calls "winfish"; this port calls it "game".
# Nothing else is changed -- these are game-logic fixes with no framework
# dependency (the file has zero PopLib references), so they apply verbatim.
...
def lf(b: bytes) -> bytes:
    """Normalise to LF. crlf() below converts the other way."""
    return b.replace(b"\r\n", b"\n")
...
def sub(path, old, new, label):
    p = DST / path
    data = p.read_bytes()
    # Some patterns in this file hardcode \r\n, because upstream's checkout of
    # the decompilation has CRLF endings. This one's does not, in every file --
    # git's autocrlf handling, the archive it came from, and which files have
    # been rewritten by earlier fixups all affect it. Normalising the pattern to
    # LF first, as well as trying it as written and as CRLF, makes the match
    # independent of all that.
    variants = ((lf(old), lf(new)), (crlf(lf(old)), crlf(lf(new))), (old, new))
    for o, n in variants:
        if o in data:
