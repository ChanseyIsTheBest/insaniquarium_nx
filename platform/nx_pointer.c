...
/* Game-supplied cursor slots. Pixels are kept until a GL context exists, then
 * uploaded lazily from nxp_draw(): SetCursorImage runs during resource loading,
 * on the loading thread, with no GL context current. */
typedef struct {
  unsigned int *argb;      /* owned copy, ARGB, w*h */
  int w, h, hot_x, hot_y;
  GLuint tex;              /* 0 until uploaded */
  int dirty;               /* pixels changed since last upload */
} NxpSlot;
/* Game cursors are drawn at this multiple of their authored size. See the use
 * site in nxp_draw for why only they are scaled. */
#define NXP_GAME_CURSOR_SCALE 2.0f

static NxpSlot s_slots[NXP_MAX_CURSOR_SLOTS];
...
static void clamp_cursor(void) {
  /* Insets keep the pointer out of the letterbox bars, where coordinates map
   * outside the game and clicks do nothing -- which reads as a freeze. */
  const float lo_x = (float)s_cfg.inset_left;
  const float lo_y = (float)s_cfg.inset_top;
  const float hi_x = (float)(s_cfg.screen_w - 1 - s_cfg.inset_right);
  const float hi_y = (float)(s_cfg.screen_h - 1 - s_cfg.inset_bottom);

  if (s_cx < lo_x) s_cx = lo_x;
  if (s_cy < lo_y) s_cy = lo_y;
  if (s_cx > hi_x) s_cx = hi_x;
  if (s_cy > hi_y) s_cy = hi_y;
}
...
void nxp_update(void) {
  ...
  if (pressed & (HidNpadButton_L | HidNpadButton_R)) {
    s_cx = (s_cfg.inset_left + (s_cfg.screen_w - s_cfg.inset_right)) * 0.5f;
    s_cy = (s_cfg.inset_top  + (s_cfg.screen_h - s_cfg.inset_bottom)) * 0.5f;
...
void nxp_set_cursor_slot(int slot, const unsigned int *argb, int w, int h,
                         int hot_x, int hot_y)
{
  if (slot < 0 || slot >= NXP_MAX_CURSOR_SLOTS || !argb || w <= 0 || h <= 0)
    return;
  NxpSlot *sl = &s_slots[slot];
  unsigned int *copy = malloc((size_t)w * (size_t)h * 4);
  if (!copy) return;
  memcpy(copy, argb, (size_t)w * (size_t)h * 4);
  free(sl->argb);
  sl->argb = copy; sl->w = w; sl->h = h; sl->hot_x = hot_x; sl->hot_y = hot_y;
  sl->dirty = 1;
  logf_("nxp: cursor slot %d set, %dx%d hotspot (%d,%d)\n", slot, w, h, hot_x, hot_y);
}

void nxp_select_cursor(int slot)
{
  if (slot < -1 || slot >= NXP_MAX_CURSOR_SLOTS) return;
  s_active_slot = slot;
}

/* Upload a slot's pixels. Needs the GL context current, so only from
 * nxp_draw(). ARGB (framework) -> RGBA (GL) swizzle happens here. */
static void slot_upload(NxpSlot *sl)
{
  if (!sl->argb || !sl->dirty) return;
  const size_t n = (size_t)sl->w * (size_t)sl->h;
  unsigned char *rgba = malloc(n * 4);
  if (!rgba) return;
  for (size_t i = 0; i < n; i++) {
    unsigned int p = sl->argb[i];
    rgba[i*4+0] = (p >> 16) & 0xFF;
    rgba[i*4+1] = (p >>  8) & 0xFF;
    rgba[i*4+2] = (p      ) & 0xFF;
    rgba[i*4+3] = (p >> 24) & 0xFF;
  }
  GLint prev_active = 0, prev_tex = 0, prev_align = 4;
  glGetIntegerv(GL_ACTIVE_TEXTURE, &prev_active);
  glActiveTexture(GL_TEXTURE0);
  glGetIntegerv(GL_TEXTURE_BINDING_2D, &prev_tex);
  glGetIntegerv(GL_UNPACK_ALIGNMENT, &prev_align);
  if (!sl->tex) glGenTextures(1, &sl->tex);
  ...
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
  glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
  glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, sl->w, sl->h, 0, GL_RGBA, GL_UNSIGNED_BYTE, rgba);
...
void nxp_draw(void) {
  ...
  /* The cursor is a screen-space overlay and its shader maps positions using
   * s_cfg.screen_w/h, i.e. the whole framebuffer. The host may have set a
   * narrower viewport -- SexyAppFramework letterboxes a 4:3 game into 1280x720
   * as glViewport(160, 0, 960, 720) -- and inheriting that would squash the
   * cursor into the letterbox and offset it from the pointer it represents.
   * Take the full framebuffer for the overlay and hand the viewport back. */
  GLint prev_vp[4];
  ...
  /* A game-supplied slot, if one is selected and set, takes precedence over
   * cursor.png and the built-in arrow. Upload happens here, lazily, because
   * this is the only place with a GL context guaranteed current. */
  for (int i = 0; i < NXP_MAX_CURSOR_SLOTS; i++)
    if (s_slots[i].dirty) slot_upload(&s_slots[i]);

  GLuint  use_tex = s_cursor_tex;
  GLfloat use_w = (GLfloat)s_cursor_w, use_h = (GLfloat)s_cursor_h;
  GLfloat hot_x = 0, hot_y = 0;                    /* cursor.png: top-left */
  if (s_active_slot >= 0 && s_slots[s_active_slot].tex) {
    const NxpSlot *sl = &s_slots[s_active_slot];
    /* Scale game cursors only.
     *
     * These are authored for a 640x480 desktop window and are much smaller than
     * the built-in arrow at 1280x720 -- Insaniquarium's pointer is 54x54 with
     * roughly 20x20 of visible artwork. The built-in arrow and cursor.png are
     * already sized for this screen and are deliberately left alone.
     *
     * The hotspot scales with the image so the tip stays under the pointer. */
    use_tex = sl->tex;
    use_w = (GLfloat)sl->w * NXP_GAME_CURSOR_SCALE;
    use_h = (GLfloat)sl->h * NXP_GAME_CURSOR_SCALE;
    hot_x = (GLfloat)sl->hot_x * NXP_GAME_CURSOR_SCALE;
    hot_y = (GLfloat)sl->hot_y * NXP_GAME_CURSOR_SCALE;
  }

  if (use_tex) {
    /* ---- textured quad, offset so the hotspot lands on the pointer ---- */
    const GLfloat w = use_w, h = use_h;
    const GLfloat quad[] = { -hot_x,   -hot_y,    w-hot_x, -hot_y,
    ...
    glBindTexture(GL_TEXTURE_2D, use_tex);
