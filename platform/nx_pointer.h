...
typedef struct {
  ...
  /* Optional: keep the cursor inside the area that actually accepts input.
   * A letterboxed game leaves black bars the pointer must not enter, because
   * coordinates there map outside the game and clicks silently do nothing.
   * All zero (the default) clamps to the full screen, as before.
   * For a 4:3 game on a 1280x720 surface: left = right = 160. */
...
/* Queries / misc */
/* Game-supplied cursors.
 *
 * SexyAppFramework games ship their own cursor artwork (pointer, hand, dragging,
 * text) and switch between them at runtime. Feed each one in as ARGB pixels
 * -- the framework's MemoryImage::GetBits() format -- and select by slot. A
 * slot that has been set overrides both cursor.png and the built-in arrow.
 *
 * The hotspot is the pixel that sits ON the pointer position. SexyAppFramework
 * cursors are drawn with the hotspot at the image centre. */
#define NXP_MAX_CURSOR_SLOTS 8
void  nxp_set_cursor_slot(int slot, const unsigned int *argb, int w, int h,
                          int hot_x, int hot_y);
void  nxp_select_cursor(int slot);   /* -1 = back to cursor.png / built-in */
